...
def generate_robot_description(context: LaunchContext, description_package,
                               use_fake_hardware, right_can_interface, ee_type):
    """Generate robot description using xacro processing.

    Configures the bimanual robot (body + right shoulder mount) with the left
    arm omitted so the URDF matches a v1.0 OpenArm with only the right arm
    physically attached.
    """
...
def robot_nodes_spawner(context: LaunchContext, description_package,
                        ...
                        right_can_interface, ee_type, arm_prefix):
    """Spawn robot_state_publisher and ros2_control_node with shared robot description."""
    namespace = namespace_from_context(context, arm_prefix)

    robot_description = generate_robot_description(
        context, description_package, use_fake_hardware,
        right_can_interface, ee_type,
    )
    ...
    if namespace:
        controllers_file_str = controllers_file_str.replace(
            "openarm_right_arm_controllers.yaml",
            "openarm_right_arm_controllers_namespaced.yaml"
        )
...
def controller_spawner(context: LaunchContext, robot_controller, arm_prefix):
    """Spawn the right-side arm controller selected by `robot_controller`."""
    namespace = namespace_from_context(context, arm_prefix)
    ...
    return [
        Node(
            ...
            executable="spawner",
            namespace=namespace,
            arguments=[robot_controller_right, "-c", controller_manager_ref],
        )
    ]


def generate_launch_description():
    """Generate launch description for the right-arm-only OpenArm v1.0 configuration."""
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            "arm_type",
            default_value="v10",
            description="Arm type. This launch supports OpenArm v1.0 only.",
        ),
        DeclareLaunchArgument(
            "use_fake_hardware",
            default_value="true",
        ...
        DeclareLaunchArgument(
            ...
            description="End-effector type attached to the right arm.",
        ),
        DeclareLaunchArgument(
            "controllers_file",
            default_value="openarm_right_arm_controllers.yaml",
